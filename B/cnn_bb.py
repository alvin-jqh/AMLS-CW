...
class modelBCNN():
    ...
    def load_dataset(self):
        ...
        Returns:
            ...
            test_loader: dataloader containing the test split
            class_weights: the weights based on the class distribution in the training split
        """

        # add random flips to the train split so for preventing overfitting
        data_transforms = {
        ...
        # compute the class weights as the dataset is imbalanced
        class_weights = compute_class_weight(class_weight='balanced', classes=np.unique(labels), y=labels)
        class_weights = torch.tensor(class_weights, dtype=torch.float)

        # the data is shuffled and placed into datalodaers, separating them insto batches
    ...
    def save_model(self, model_filename="cnn_model_weights.pt"):
        ...
        model_path = os.path.join(os.getcwd(),"B", model_filename)
    ...
    def setup_model(self, load=True):
        ...
        """
        # find all the files in the B directory with extension .pt
        files = [file for file in os.listdir(os.path.join(os.getcwd(),"B")) if file.endswith(".pt")]
        ...
        if load:
            ...
            file_path = os.path.join(os.getcwd(), "B", filename)
    ...
    def test_accuracy(self):
        ...
        with torch.no_grad():
            ...
            evaluator = Evaluator("bloodmnist", "test")
...
def load_test():
    ...
    if action == 1:
        ...
        weighted_choice = int(input("\nWeighting off classes:\n1. no weights\n2. Balanced Weights\n").strip())
    ...
    bb = modelBCNN(BATCH_SIZE=64, load_model=load_model, model_type=model_choice, patience=8, delta=0.01)
    ...
    if action == 1:
        ...
        if optimiser_choice == 1:
            optimizer_ft = optimizers[optimiser_choice-1](bb.model.parameters(), lr=learning_rate, momentum = momentum)
        else:
            optimizer_ft = optimizers[optimiser_choice-1](bb.model.parameters(), lr=learning_rate)
        ...
        bb.train_model(optimizer=optimizer_ft, scheduler=scheduler,num_epochs=epochs, weighted= weighted)
    
    bb.visualize_model()
    bb.test_accuracy()

    # only ask to save if trained
    if action == 1:
